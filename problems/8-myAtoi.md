...
4. **Rounding**:
   - If the integer is out of the 32-bit signed integer range `[-2^31, 2^31 - 1]`, round the integer to remain in this range:
     - Integers less than `-2^31` are rounded to `-2^31`.
     - Integers greater than `2^31 - 1` are rounded to `2^31 - 1`.
